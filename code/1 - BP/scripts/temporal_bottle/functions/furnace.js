import { BlockComponentTypes, ItemTypes } from "@minecraft/server";
...
export const temporalBottleFuncFurnace = new class TemporalBottleFuncFurnace {
    smelt(block) {
        ...
        if (!inputSlot)
            return;
        if (!(furnaceHasRecipe[block.typeId.replace("lit_", "")]?.has(inputSlot.typeId) ?? false))
            return;
        ...
        if (Array.isArray(expectedOutput)) {
            ...
            if (output?.typeId != expectedOutput && output != undefined)
                return;
            if (ItemTypes.get(output?.typeId ?? expectedOutput))
                outputSlot = apiItemAmount.increase(output ?? expectedOutput, 1);
        }
        if (!outputSlot)
            outputSlot = false;
        if (outputSlot == false)
